...
#[test]
fn grace_network_reward_becomes_claimable_after_timely_reactivation() {
    ...
    let activate_ix = ctx
        ...
        .accounts(service_referral_protocol::accounts::PurchaseServiceUnits {
            ...
            batch: sponsor_batch_0,
            token_program: spl_token::id(),
            system_program: anchor_lang::system_program::ID,
        })
        .args(service_referral_protocol::instruction::PurchaseServiceUnits { units: 10 })
        .instruction()
        .expect("activate sponsor ix");
    ctx.execute_instruction(activate_ix, &[&sponsor])
        .expect("activate sponsor")
        .assert_success();
    ...
    let reactivate_ix = ctx
        ...
        .accounts(service_referral_protocol::accounts::PurchaseServiceUnits {
            ...
            service_treasury_usdt: treasury_usdt,
            service_treasury_usdc: treasury_usdc,
            batch: sponsor_batch_1,
            token_program: spl_token::id(),
            system_program: anchor_lang::system_program::ID,
        })
        .args(service_referral_protocol::instruction::PurchaseServiceUnits { units: 10 })
        .instruction()
        .expect("reactivate sponsor ix");
    ctx.execute_instruction(reactivate_ix, &[&sponsor])
        .expect("reactivate sponsor")
        .assert_success();

    let sponsor_account = ctx.svm.get_account(&sponsor_pda).expect("sponsor after reactivation");
    let mut sponsor_data = sponsor_account.data.as_slice();
    let sponsor_state = UserState::try_deserialize(&mut sponsor_data).expect("deserialize sponsor after reactivation");
    assert_eq!(sponsor_state.network_pending_usdc, 0);
    assert_eq!(sponsor_state.network_claimable_usdc, 15 * UNIT);
